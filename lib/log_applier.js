...
LA.persist = function persist(cb) {
  ...
  if (state.commitIndex > state.lastApplied) {
    ...
    if (! entry) throw new Error('log applier: no entry at log index', toApply);
    if (entry.topologyChange) {
      // this is an internal command, a topology command that was already
      // processed (topology commands are processed once they are inserted into the log):
