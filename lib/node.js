...
N.join = function join(peerDesc, cb) {
  ...
  async.series([ensureLeader, pushAddPeerCommand], cb);
...
N.applyTopologyChange = function applyTopologyChange(entry) {
  var fn = topologyChangeCommands[entry.shift()];
  if (fn) fn.apply(this, entry);
};
...
N.command = function command(cmd, cb, isTopologyChange) {
  ...
  ensureLeader(function(err) {
    ...
    if (err) cb(err);
      ...
      this.commonState.persisted.log.push(entry);
