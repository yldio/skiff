...
class Node extends EventEmitter {
  ...
  _dispatch () {
    ...
    if (!message) {
      ...
      if (message.from === this.id.toString()) {
        this.emit('error', new Error(`Got message from self: ${JSON.stringify(message)}`))
        return
      }

      if (this._peers.indexOf(message.from) < 0) {
        debug('%s: got message from unknown node %s', this.id, message.from)
  ...
  command (command, options, done) {
    if (this._stateName !== 'leader') {
      ...
      if (command.type === 'join') {
        if (this._peers.indexOf(command.peer) < 0) {
  ...
  _applyTopologyCommand (command) {
    ...
    if (command.type === 'join') {
      if ((command.peer !== this.id.toString()) && (this._peers.indexOf(command.peer) === -1)) {
      ...
      this.emit('left', command.peer)
    }
    debug('%s: peers are: %j', this.id, this._peers)
  }
