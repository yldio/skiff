...
class Node extends EventEmitter {
  ...
  constructor (id, connections, dispatcher, db, peers, options) {
    ...
    this._client = new Client({
      ...
      leaderBase: this._getLeaderBase.bind(this),
  ...
  _ensurePeer (address) {
    if ((this._peers.indexOf(address) < 0) && address !== this.id.toString()) {
      ...
      console.log('%s is joining %s', this.id, address)
  ...
  _transition (state, force) {
    ...
    if (force || state !== this._stateName) {
      ...
      // console.log('node %s is transitioning to state %s', this.id, state)
      ...
      this._state = new State({
        ...
        log: this._log,
        command: this.command.bind(this),
        resetLeader: this._resetLeader.bind(this),
        leader: this._getLeader.bind(this),
        leaderBase: this._getLeaderBase.bind(this)
      }, this._options)
  ...
  _dispatchMessage (message) {
    ...
    if (message.params) {
      if (message.action !== 'Command' && message.params.term < this._term) {
      ...
      if (message.params.leaderId) {
        this._setLeader(message.params.leaderId)
      }
  ...
  _handleRequest (message, done) {
    ...
    if (from) {
      debug('%s: handling request message: %j', this.id, message)
  ...
  _write (message, done) {
    if (this.active) {
      this.active.write(message, done)
    } else {
      done()
    }
  }
  ...
  _resetLeader () {
    this._leaderId = this._leaderBase = undefined
  }
  ...
  _getLeaderBase () {
    return this._leaderBase
  }

  _setLeader (leaderId) {
    this._leaderId = leaderId
    this._leaderBase = leaderId.split('/').slice(0, 5).join('/')
  }
  ...
  command (command, options, done) {
    // console.log('%s command: %j', this.id, command)
    if (this._stateName !== 'leader') {
