...
function Node(options) {
  ...
  function loaded(err) {
    ...
    self.emit('loaded');
    self.toState(options.standby ? 'standby' : 'follower');
  }
}
...
N.leave = function join(peerDesc, cb) {
  ...
  async.series([
    ensureLeader,
    pushWillRemovePeerCommand,
    pushRemovePeerCommand], cb);

  function ensureLeader(cb) {
    self.ensureLeader(cb);
  }

  function pushWillRemovePeerCommand(cb) {
    var cmd = ['will remove peer', peerDesc];
    self.command(cmd, cb, true);
  }
...
N._willLeave = function _willLeave(peerDesc) {
