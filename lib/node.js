...
N.load = function load(cb) {
  ...
  function newPeer(peerDesc) {
    var peer = new Peer(peerDesc, self.options);
    self._join(peer);
...
N.save = function save(cb) {
  this.options.persistence.saveMeta(this.id, this.commonState.persisted, cb);
};
...
N.listen = function listen(options) {
  var self = this;
  this.options.transport.listen(options, listener);

  function listener(peerId, connection) {
    var joined = self._join(peerId, connection);
    if (!joined) {
      connection.close();
    }
  }
  ...
  if (peerDesc != this.id) {
    ...
    if (!found) {
      ...
      if (!(peerDesc instanceof Peer)) {
        peer = new Peer(peerDesc, this.options, connection);
      }
...
N.command = function command(cmd, options, cb, isTopologyChange) {
  ...
  function replicate(done) {
    ...
    self.state.replicate(commitIndex, options.timeout, done);
  }
...
N.onAppendEntries = function onAppendEntries(args, cb) {
  var self = this;
  self.emit('AppendEntries', args);
  this.state.onAppendEntries(args, cb);
};

N.onRequestVote = function onRequestVote(args, cb) {
  var self = this;
  self.emit('RequestVote', args);
  this.state.onRequestVote(args, cb);
};
...
N.stop = function stop(cb) {
  ...
  if (!this.stopped) {
    ...
    this.state.once('stopped', onceStateStopped);
    this.state.stop();
  } else if (cb) {
    setImmediate(cb);
  }

  function onceStateStopped() {
