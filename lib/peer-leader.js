...
class PeerLeader extends EventEmitter {
  ...
  needsIndex (index) {
    if (index > this._needsIndex) {
      console.log('needs index %d', index)
      this._needsIndex = index
    }
    debug('%s: peer %s needs index %d', this._node.state.id, this._address, index)
    if (this._needsMore()) {
      debug('%s: peer %s needs more append entries to reach index %d', this._node.state.id, this._address, index)
      timers.setImmediate(this._appendEntries.bind(this))
    } else {
      debug('%s: peer %s does NOT need more append entries to reach index %d', this._node.state.id, this._address, index)
  ...
  _needsMore () {
    ...
    console.log('%s: %s needs more? %j', this._node.state.id, this._address, needs)
    return needs
  }

  _appendEntries () {
    debug('sending AppendEntries to %s', this._address)
    console.log('sending AppendEntries to %s', this._address)
    ...
    if (entries) {
      ...
      this._node.network.rpc(
        ...
        (err, reply) => { // callback
          ...
          console.log('%s: got reply to AppendEntries from %s: %j', this._node.state.id, this._address, reply, err && err.stack)
          if (err) {
            ...
            if (reply.params.success) {
              ...
              if (lastEntry) {
                this._matchIndex = lastEntry.i
                console.log('_match index now is %s', this._matchIndex)
        ...
        (err, reply) => { // callback
          ...
          if (err) {
            ...
            if (data.finished) {
              ...
              cleanup()
              self.emit('committed', self, lastIncludedIndex)
              if (self._needsMore()) {
                timers.setImmediate(self._appendEntries.bind(self))
              }
            } else {
