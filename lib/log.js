...
class Log {
  ...
  _compact () {
    if (this._entries.length > this._options.maxLogRetention) {
      const maxPhysicalIndex = this._entries.length - this._options.maxLogRetention
...
function cleanupEntry (_entry) {
  ...
  if (entry.c) {
    if (Array.isArray(entry.c)) {
      entry.c = entry.c.map(cleanupCommand)
    } else {
      entry.c = cleanupCommand(entry.c)
    }
  }
  return entry
}

function cleanupCommand (_command) {
  const command = Object.assign({}, _command)
  if (command.prefix) {
