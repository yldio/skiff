...
function SkiffClient(id, options) {
  ...
  function work(item, cb) {
    ...
    if (beforeLeaderOps.indexOf(item.op) == -1 &&
      ...
      callback(new Error(item.op + ' operation requires being the leader'));
    } else {
      args.push(callback);
...
SC.join = function join(node, cb) {
  this.node.join(node, cb);
};
...
SC._waitLeader = function _waitLeader(cb) {
  ...
  if (this.node.state.name == 'leader') {
    callback();
  } else {
    setTimeout(callback, this._options.waitLeaderTimeout);
    this.node.once('leader', callback);
  }
