...
class Leader extends Base {
  ...
  command (consensuses, command, options, done) {
    ...
    process.nextTick(() => {
      async.each(consensuses, this._waitForConsensus.bind(this, index, options), (err) => {
        if (err) {
          done(err)
        } else {
  ...
  _waitForConsensus (waitingForIndex, options, consensus, _done) {
    ...
    let waitingFor = (options.alsoWaitFor !== this.id) && options.alsoWaitFor
    ...
    function onPeerCommit (peer, peerIndex) {
      if (peerIndex >= waitingForIndex) {
        ...
        if (waitingFor && peer._address === waitingFor) {
          waitingFor = null
        }
      }
      if (isMajority(consensus, votes) && !waitingFor) {
