...
class Leader extends Base {
  ...
  join (address) {
    ...
    if (!follower) {
      this._createPeerLeader(address)
    }
  }

  _createPeerLeader (address) {
    const peerLeader = new PeerLeader(address, this._node, this._options)
    this._followers[address] = peerLeader
    peerLeader.needsIndex(this._lastIndex)
    return peerLeader
  }
  ...
  command (consensuses, command, options, done) {
    const index = this._lastIndex = this._node.log.push(command)
  ...
  _waitForConsensus (waitingForIndex, options, consensus, _done) {
    debug('_waitForConsensus %d', waitingForIndex, consensus)
    ...
    const peers = consensus.map(address => {
      ...
      if (!follower) {
        follower = this._createPeerLeader(address)
      }
      return follower
    })
