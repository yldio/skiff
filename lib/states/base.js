...
const encode = require('json-stringify-safe')
...
class Base extends EventEmitter {
  ...
  handleRequest (message, done) {
    ...
    switch (message.action) {
      ...
      case 'Quit':
        this._quitReceived(message, done)
        break

      case 'Ping':
        this._pingReceived(message, done)
        break
  ...
  _appendEntriesReceived (message, done) {
    console.log('append entries received')
    const self = this
  ...
  _pingReceived (message, done) {
    debug('%s: _pingReceived %j', this.id, message)

    if (message.from === this._node.leaderBase()) {
      debug('%s: ping from %s matched leader base', this.id, message.from)
      this._resetElectionTimeout()
    } else {
      debug('%s: ping ignored because leader is %j', this.id, this._node.leaderBase())
    }
    done()
  }

  _quitReceived (message, done) {
    debug('%s: _quitReceived %j', this.id, message)
    if (message.from === this._node.leader()) {
      debug('%s: leader quit, resetting leader', this.id)
      this._node.resetLeader()
    }
    done()
  }
